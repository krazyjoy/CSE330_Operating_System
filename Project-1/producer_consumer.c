...
int producer_thread_function(void *pv)
{
	...
	for_each_process(task)
	{
		if (task->cred->uid.val == uuid)
		{
			...
			down(&empty);
			down(&mutex);
			//pr_info("%s: Producer -> Semaphore acquired. Accessing shared resource...\n", current->comm);
			...
			if(fill<buffSize){
				...
				fill = (fill +1 ) % buffSize; // increase fill by one 
			...
			//pr_info("%s: Producer -> Releasing semaphore and finishing...\n", current->comm);
...
int consumer_thread_function(void *pv)
{
	...
	while (!kthread_should_stop())
	{
		...
		//pr_info("%s: Consumer -> Semaphore acquired. Accessing shared resource...\n", current->comm);
		int process_pid = buffer[use].pid;


		unsigned long long start_time_ns = buffer[use].start_time;
		// boot_time_ns = buffer[use].boot_time;


		// ADD UP PROCESS TIME TO TOTAL TIME
		...
		use = (use + 1) % buffSize; // increase use by 1
		...
		up(&mutex);
		up(&empty);
		// pr_info("%s: Consumer -> Releasing semaphore and finishing...\n", current->comm);
...
static int __init thread_init_module(void)
{
	...
	if (buffSize > 0 && (prod >= 0 && prod < 2))
	{
		...
		sema_init(&full, 0);
		sema_init(&mutex, 1); // only 1 reader at one time
		name_threads();
		pr_info("------------------ buffer size is ----------------------- %d ", buffSize);
		...
		for (int prod_id=0; prod_id<prod; prod_id++){

			//char[] prod_name = strcat("kproducer-", )
			ctx_producer_thread[prod_id] = kthread_run(producer_thread_function, NULL, producers[prod_id]);
		...
		for(int con_id=0; con_id<cons; con_id++){
			ctx_consumer_thread[con_id]= kthread_run(consumer_thread_function, NULL, consumers[con_id]);
			// pr_info("set end flag");
		}
